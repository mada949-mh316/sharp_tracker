...
def fetch_dfs_from_db(days_back: int) -> pd.DataFrame:
    ...
    time_clause = ""
    if days_back > 0:
        time_clause = "AND timestamp >= NOW() - INTERVAL '%s days'"
    ...
    def _platform(book):
        b = str(book).lower()
        if 'prizepicks' in b: return 'PrizePicks'
        if 'underdog' in b:   return 'Underdog'
        if 'pick6' in b or 'draftkings6' in b or 'dk6' in b: return 'Pick6'
        if 'sleeper' in b:    return 'Sleeper'
        return book
    df['platform'] = df['play_book'].apply(_platform)
    # parse over/under direction and line from play_selection
    def _direction(sel):
        if ' Over '  in str(sel): return 'Over'
        if ' Under ' in str(sel): return 'Under'
        return 'Other'
    def _player(sel):
        s = str(sel)
        for kw in [' Over ', ' Under ']:
            if kw in s:
                return s.split(kw)[0].strip()
        return s
    def _market_clean(m):
        return str(m).replace('Player Prop', '').replace('Player prop', '').strip(' -–').strip()
    df['direction']    = df['play_selection'].apply(_direction)
    df['player']       = df['play_selection'].apply(_player)
    df['market_clean'] = df['market'].apply(_market_clean)
    return df
...
def bar(df_plot, x, y, title="", h=280, text_fmt='roi'):
    ...
    if 'n' in df_plot.columns:
        texts = [f"{v:+.1f}%  N={int(n)}" if text_fmt == 'roi' else f"${v:,.0f}  N={int(n)}"
                 for v, n in zip(df_plot[y], df_plot['n'])]
    else:
...
def hbar(df_plot, x, y, title="", h=300):
    ...
    if 'n' in df_plot.columns:
        texts = [f"{v:+.1f}%  N={int(n)}" for v, n in zip(df_plot[x], df_plot['n'])]
    else:
        texts = [f"{v:+.1f}%" for v in df_plot[x]]
...
with tab_props:
    ...
    if props_closed.empty:
        ...
        fig_x = px.bar(cross,x='prop_cat',y='roi',color='bet_side',barmode='group',
            color_discrete_map={'Over':'#f87171','Under':'#4ade80','Other':'#e6edf3'},text_auto='.1f')
...
with tab_edge:
    ...
    if HAS_MY_SCORE and not settled_e.empty:
        ...
        def _bms_chart(data, key_col, title, chart_key):
            ...
            combo_s = combo_s[combo_s['n'] >= bms_min].sort_values('roi', ascending=False).head(20)
            if combo_s.empty:
...
with tab_parlays:
    ...
    if len(date_range) == 2 and not pdf.empty:
        pdf = pdf[(pdf['created_at'].dt.date >= date_range[0]) & (pdf['created_at'].dt.date <= date_range[1])]
    ...
    if pdf.empty:
        ...
        if not settled.empty:
            ...
            # ── By EV range ─────────────────────────────────────
            ev_settled = settled[settled['ev_pct'].notna()].copy()
            if not ev_settled.empty:
                st.markdown("**Performance by EV Range**")
                ev_bins   = [-999, 0, 5, 10, 20, 999]
                ev_labels = ["< 0%", "0–5%", "5–10%", "10–20%", "> 20%"]
                ev_settled['ev_bucket'] = pd.cut(ev_settled['ev_pct'], bins=ev_bins, labels=ev_labels)
                ev_rows = []
                for lbl in ev_labels:
                    sub = ev_settled[ev_settled['ev_bucket'] == lbl]
                    if sub.empty: continue
                    n_w = (sub['status'] == 'Won').sum()
                    n_l = (sub['status'] == 'Lost').sum()
                    pnl = sub['profit'].sum()
                    roi = pnl / (len(sub) * 100) * 100
                    ev_rows.append({'EV Range': lbl, 'Count': len(sub), 'W': n_w, 'L': n_l,
                                    'Win%': f"{n_w/len(sub)*100:.0f}%",
                                    'ROI': f"{roi:+.1f}%", 'P&L': f"${pnl:+,.0f}",
                                    '_roi': roi, '_pnl': pnl / 100})
                if ev_rows:
                    ev_df = pd.DataFrame(ev_rows)
                    st.dataframe(ev_df.drop(columns=['_roi', '_pnl']),
                                 use_container_width=True, hide_index=True)
                    ev_col1, ev_col2 = st.columns(2)
                    with ev_col1:
                        fig_ev_roi = px.bar(ev_df, x='EV Range', y='_roi',
                                            labels={'EV Range': '', '_roi': 'ROI (%)'},
                                            color='_roi',
                                            color_continuous_scale=['#E74C3C', '#F39C12', '#2ECC71'],
                                            color_continuous_midpoint=0)
                        fig_ev_roi.update_layout(**LAYOUT, title="ROI by EV Range", height=300,
                                                 showlegend=False, coloraxis_showscale=False)
                        fig_ev_roi.add_hline(y=0, line_dash='dash', line_color='#30363d')
                        st.plotly_chart(fig_ev_roi, use_container_width=True)
                    with ev_col2:
                        fig_ev_pnl = px.bar(ev_df, x='EV Range', y='_pnl',
                                            labels={'EV Range': '', '_pnl': 'Profit (units)'},
                                            color='_pnl',
                                            color_continuous_scale=['#E74C3C', '#F39C12', '#2ECC71'],
                                            color_continuous_midpoint=0)
                        fig_ev_pnl.update_layout(**LAYOUT, title="Total Profit by EV Range", height=300,
                                                  showlegend=False, coloraxis_showscale=False)
                        fig_ev_pnl.add_hline(y=0, line_dash='dash', line_color='#30363d')
                        st.plotly_chart(fig_ev_pnl, use_container_width=True)
...
with tab_dfs:
    ...
    if len(date_range) == 2 and not dpdf.empty:
        dpdf = dpdf[(dpdf['created_at'].dt.date >= date_range[0]) & (dpdf['created_at'].dt.date <= date_range[1])]
